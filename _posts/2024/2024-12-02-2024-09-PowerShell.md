...
```
* 2) Geez that sounds ominous, I'm sure we can get past the defense mechanisms.<br/>We should warm up our PowerShell skills.<br/>How many words are there in the file?
```powershell
...
```
* 5) It looks like defensive measures are in place, it is protected by basic authentication.<br/>Try authenticating with a standard admin username and password.
```powershell
...
RelationLink      : {}
```
* 6) There are too many endpoints here.<br/>Use a loop to download the contents of each page. What page has 138 words?<br/>When you find it, communicate with the URL and print the contents to the terminal.
```powershell
...
```
* 7) There seems to be a csv file in the comments of that page.<br/>That could be valuable, read the contents of that csv-file!
```powershell
$creds = New-Object System.Management.Automation.PSCredential("admin", (ConvertTo-SecureString "admin" -AsPlainText -Force))
...
```
* 8) Luckily the defense mechanisms were faulty!<br/>There seems to be one api-endpoint that still isn't redacted! Communicate with that endpoint!
```powershell
$creds = New-Object System.Management.Automation.PSCredential("admin", (ConvertTo-SecureString "admin" -AsPlainText -Force))
...
```
* 10) Sweet we got a MFA token! We might be able to get access to the system.<br/>Validate that token at the endpoint!
...
```
* 12) Hurray! You have thwarted their defenses!<br/>Alabaster can now access their weaponry and put a stop to it.<br/>Once HHC grants your achievement, you can close this terminal.
...
[![09_99_powershell_just_dont.png](/assets/static/posts/2024/2024-09-PowerShell/09_99_powershell_just_dont.png)](https://www.reddit.com/r/linuxmemes/comments/8dnw9z/how_i_feel_about_powershell_right_now/)
